...
public class StellartechModVillagerProfessions {
	...
	public static final RegistryObject<VillagerProfession> HACKER = registerProfession("hacker", () -> StellartechModItems.HACKERTABLE.get(), () -> ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("stellartech:typer_short")));
	...
	private static RegistryObject<VillagerProfession> registerProfession(String name, Supplier<Block> block, Supplier<SoundEvent> soundEvent) {
		...
		return PROFESSIONS.register(name, () -> {
			...
			return new VillagerProfession(StellartechMod.MODID + ":" + name, poiPredicate, poiPredicate, ImmutableSet.of(), ImmutableSet.of(), soundEvent.get());
		});
	}
		...
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
					StellartechMod.LOGGER.error("Skipping villager profession " + name + " that uses POI block " + block + " that is already in use by " + existingCheck);
					continue;
				}
